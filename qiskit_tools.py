...
def QFTMultiply(circ, qreg1, qreg2, qreg3, A=1., wrap=False, inverse=False, nint=None, label='Mult', QFT_on=True, iQFT_on=True):
    r"""
...
def QFTMultPhase(circ, qreg1, qreg2, qreg3, wrap=False, inverse=False, nint=None, label='MultPhase', QFT_on=True, iQFT_on=True):
    r"""
    |qreg1>|qreg2>|qreg3> -> |qreg1>|qreg2>|qreg1 x qreg2>
    """
    n1 = len(qreg1)
    ...
    comp_gate2_inv = TwosCompliment(circ, qreg2[:-1], wrap=True, inverse=True).control(1)
    circ.append(comp_gate2_inv, [qreg2[-1], *qreg2[:-1]]);

    mult_gate = QFTMultiply(circ, qreg1[:-1], qreg2[:-1], qreg3[:-1], nint=nint, wrap=True)
    circ.append(mult_gate, [*qreg1[:-1], *qreg2[:-1], *qreg3[:-1]]);

    circ.x(qreg1[-1]);
    comp_gate = TwosCompliment(circ, qreg3[:-1], wrap=True).control(2)
    circ.append(comp_gate, [qreg1[-1], qreg2[-1], *qreg3[:-1]]);
    circ.x(qreg1[-1]);
    ...
    comp_gate2 = TwosCompliment(circ, qreg2[:-1], wrap=True).control(1)
    circ.append(comp_gate2, [qreg2[-1], *qreg2[:-1]]);

    for qubit in np.arange(n1):
        circ.x(qreg1[qubit]);
    for qubit in np.arange(n2-1):
        circ.x(qreg2[qubit]);
...
def QFTPowerN(circ, qreg1, qreg2, N, wrap=False, inverse=False, nint=None, nshift=0, label='PowN', QFT_on=True, iQFT_on=True):
    r"""
    ...
    gridouts = 2*np.pi/(np.power(2.,np.sum(qubit_grid,axis=0)+gridouts.T-n2-(nint+nshift)))
