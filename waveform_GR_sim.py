...
def sim_waveform_GR(nx, nlab, ncut0, ncut1, nsig0, nsig1, statename='./state_vectors/psi_f', fmin=40., fmax=168., m1=35., m2=30., Tfrac=100., beta=0., sig=0.):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--statename', help="Numpy file with saved state vector.", default='./state_vectors/full_state')
    ...
    sim_waveform_GR(opt.nx, opt.nlab, statename=opt.statename, ncut0=opt.ncut0, ncut1=opt.ncut1, nsig0=opt.nsig0, nsig1=opt.nsig1, fmin=opt.fmin, fmax=opt.fmax, m1=opt.m1, m2=opt.m2, Tfrac=opt.Tfrac, beta=opt.beta, sig=opt.sig)
    
    plsim.plot_sim(opt.statename+'.npy', './figures/full_state.png', r'\Psi(f)', fmin=opt.fmin, fmax=opt.fmax, m1=opt.m1, m2=opt.m2, Tfrac=opt.Tfrac, beta=opt.beta, sig=opt.sig)
