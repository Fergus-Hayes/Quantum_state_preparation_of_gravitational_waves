...
def sim_waveform_QPC(nx, nlab, ncut0, ncut1, nsig0, nsig1, statename='./state_vectors/psi_f', fmin=40., fmax=168., m1=35., m2=30., Tfrac=100., beta=0., sig=0.):
    ...
    weights = np.load('./weights_20_2260.npy')
    ...
    phase_gates = qt.qubit_to_phase(circ, q_y, nint, wrap=True, phase=phase, factor=1)
    circ.append(phase_gates, q_y);

    func_gate_inv = qt.piecewise_function_posmulti(circ, q_x, q_y, q_lab, q_coff, coeffs, bounds, nint=nint, nintx=nintx, nintcs=nintcs, phase=phase, wrap=True, unlabel=False, unfirst=False, inverse=True)
    circ.append(func_gate_inv, [*q_x, *q_y, *q_lab, *q_coff])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--statename', help="Numpy file with saved state vector.", default='./state_vectors/full_state_QGAN')
    ...
    sim_waveform_QPC(opt.nx, opt.nlab, statename=opt.statename, ncut0=opt.ncut0, ncut1=opt.ncut1, nsig0=opt.nsig0, nsig1=opt.nsig1, fmin=opt.fmin, fmax=opt.fmax, m1=opt.m1, m2=opt.m2, Tfrac=opt.Tfrac, beta=opt.beta, sig=opt.sig)
    
    plsim.plot_sim(opt.statename+'.npy', './figures/QGAN_full_state.png', r'\Psi(f)', fmin=opt.fmin, fmax=opt.fmax, m1=opt.m1, m2=opt.m2, Tfrac=opt.Tfrac, beta=opt.beta, sig=opt.sig)
